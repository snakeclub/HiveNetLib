...
"""
连接池处理框架
@module connection_pool_fw
@file connection_pool_fw.py
"""
...
__MOUDLE__ = 'connection_pool_fw'  # 模块名
...
class ConnectionPoolFw(ABC):
    ...
    _access_lock = threading.Lock()  # 访问连接池对象的线程锁
    ...
    _free_pool = MemoryQueue(queue_type=EnumQueueType.LIFO, maxsize=0)

    _is_destroyed = False  # 标识连接池是否已被销毁
    _daemon_running = True   # 标识守护线程是否在执行

    #############################
    # 公共属性
    #############################
    connect_para = NullObj()
    name = 'ConnectionPool'
    maxsize = 100
    minsize = 0
    ...
    def reconnect(self, connection):
        ...
        """
        return self._reconnect(connection)
    ...
    def _test_connection(self, connection):
        ...
        with ExceptionTool.ignored(
            expect=(), logger=self._logger,
            self_log_msg='Connection Pool [%s] test connection[%s] Error ' % (
                self.name, connection.connection_pool_id),
            force_log_level=logging.ERROR
        ):
        ...
        while True:
            ...
            with ExceptionTool.ignored_all(
                ...
                for (_key, _value) in self._connection_pool.items():
                    ...
                    if not _is_realse and self.test_while_idle and (_now - _value.last_test_time).total_seconds() > self.realse_free_time:
