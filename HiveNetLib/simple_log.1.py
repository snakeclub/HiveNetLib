...
class Logger(object):
    ...
    def __init__(self, conf_file_name='logger.json', logger_name='root', logfile_path='',
        ...
        @param {string} conf_file_name='logger.json' - 日志配置文件路径和文件名:
            默认为'logger.json'，如果找不到配置文件本函数会自动创带默认设置的配置文件
        @param {string} logger_name='root' - 输出日志类型，根据conf配置可以新增自定义类型，默认为'root':
            root-输出到屏幕,File-输出到文件,ConsoleAndFile-同时输出到屏幕和文件
        @param {string} logfile_path='' - 如果已有配置文件的情况该参数无效
            日志输出文件的路径（含文件名），''代表使用'log/程序名.log'
        ...
        # TODO(黎慧剑): 补充如果类型XML_FILE的处理，要转换为JSON格式统一处理
...
class handler_memoryQueueHandler(logging.Handler):
    """
    内存队列日志处理Handler类
    将日志信息写入全局变量里的指定队列对象，每个传入的日志的对象信息如下：
        queue_obj.topic_name {string} : 日志主题标识（用于使用方区分日志来源）
        queue_obj.msg {string} : 日志内容
    @example
        日志配置信息如下，其中topic_name可以不传值：
        [handler_memoryQueueHandler]
        class=HiveNetLib.simple_log.handler_memoryQueueHandler
        level=DEBUG
...
class handler_gRPCHandler(logging.Handler):
    """
    gRPC日志处理Handler类
    将日志信息通过gRPC调用传输到远程日志服务中
    @example
        日志配置信息如下，其中topic_name可以不传值：
        [handler_gRPCHandler]
        class=HiveNetLib.simple_log.handler_gRPCHandler
        level=DEBUG
        formatter=logstashFormatter
        args=("queue_var_name", "topic_name")
...
# TODO(黎慧剑): 增加通过队列异步写日志文件的类支持


# TODO(黎慧剑): 增加gRPC日志服务端应用类
